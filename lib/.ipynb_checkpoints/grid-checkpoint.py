...
class Grid:
    ...
    def get_target(self) -> List[Coordinate]:
        return np.argwhere(self.grid == CellType.TARGET)[0]
    
    def get_obstacles(self) -> List[Coordinate]:
        return list(map(tuple, np.argwhere(self.grid == CellType.OBSTACLE)))
